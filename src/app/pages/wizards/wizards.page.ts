...
export class WizardsPage implements OnInit {
  ...
  protected filteredWizards = computed(() => {
    const search = this.searchService.getActiveSearchQuery('wizard')().toLowerCase().trim();
    ...
    return wizards.filter(wizard =>
      wizard.name.toLowerCase().includes(search)
    );
  });

  constructor(private hpApiService: HpApiService, private searchService: SearchbarService) {}
